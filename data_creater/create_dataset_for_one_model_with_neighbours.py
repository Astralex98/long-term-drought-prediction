...
class OnePointDatasetCreater:
    
    def __init__(self, raw_data_for_one_point, history_len, num_of_future_indexes, pdsi_threshold = None):
    ...
    def create_dataset_from_series(self):
        ...
        if (self.pdsi_threshold != None):
            labels = (data_for_one_point[:, -self.num_of_future_indexes:] < self.pdsi_threshold).astype(int)
...
class OnePointWithNeghboursDatasetCreater:
    
    def __init__(self, raw_data_for_region, x_cur, y_cur, filter_size, history_len, num_of_future_indexes, 
                 pdsi_threshold = None):
    ...
    def create_dataset_with_neighbours_info(self):
        ...
        obj = OnePointDatasetCreater(self.raw_data_for_region[:, self.x_cur, self.y_cur], self.history_len,
                                     self.num_of_future_indexes, self.pdsi_threshold)
        
        # Separate data and labels for current point
        data_with_labels_cur_point = obj.create_dataset_from_series()
        data_for_cur_point = data_with_labels_cur_point[:, :-self.num_of_future_indexes]
        ...
        for neighbour in neighbours:
            ...
            obj = OnePointDatasetCreater(self.raw_data_for_region[:, x, y], self.history_len,
                                         self.num_of_future_indexes, self.pdsi_threshold)
            data_for_neighbour = obj.create_dataset_from_series()
            
            # Concatenating histories for current point and neighbour
            data_for_cur_point = np.concatenate((data_for_cur_point, data_for_neighbour[:, :-self.num_of_future_indexes]), 
                                                axis=1)
        ...
        if (len(neighbours) < filter_width * filter_height - 1):
            ...
            zeroes_num = (filter_width * filter_height - len(neighbours) - 1) * self.history_len
            zeroes_mas = np.zeros((data_for_cur_point.shape[0], zeroes_num))
            data_for_cur_point = np.concatenate((data_for_cur_point, zeroes_mas), axis=1)
        ...
        true_cur_point_shape = self.history_len * filter_width * filter_height + self.num_of_future_indexes
...
class AllPointsDatasetCreater:
    
    def __init__(self, raw_data_for_all_points, history_len, num_of_future_indexes, time_border,
                x_min, x_max, y_min, y_max, filter_size, pdsi_threshold = None):
        
        # Iterating model over [x_min, x_max)x[y_min, y_max) points
        assert_x_msg = "X coordinate should be in range [0, {})".format(raw_data_for_all_points.shape[1])
        assert_y_msg = "Y coordinate should be in range [0, {})".format(raw_data_for_all_points.shape[2])
        assert 0<=x_min<raw_data_for_all_points.shape[1] and 0<x_max<=raw_data_for_all_points.shape[1], assert_x_msg
        assert 0<=y_min<raw_data_for_all_points.shape[2] and 0<y_max<=raw_data_for_all_points.shape[2], assert_y_msg
        ...
        self.raw_data_for_all_points = raw_data_for_all_points[:, x_min:x_max,y_min:y_max]
    ...
    def create_train_and_test_datasets(self):
        ...
        times = [ind for ind in range(self.history_len - 1, self.raw_data_for_all_points.shape[0] - self.num_of_future_indexes)]
        ...
        for x in range(x_min, x_max):
            for y in range(y_min, y_max):
                ...
                obj = OnePointWithNeghboursDatasetCreater(self.raw_data_for_all_points, x, y, self.filter_size, 
                                                          self.history_len, self.num_of_future_indexes,
                                                          self.pdsi_threshold)
                ...
                # with training and test datasets for previous points.
                    ...
                    self.train_array = np.concatenate((self.train_array, data_for_one_point[:idx]), axis=0)
                    self.test_array = np.concatenate((self.test_array, data_for_one_point[idx:]), axis=0)
    ...
    def get_train_array_by_points(self):
        ...
        assert len(self.train_array_by_points)==(self.x_max - self.x_min)*(self.y_max - self.y_min), assert_msg
        
        return self.train_array_by_points
    
    def get_test_array_by_points(self):
        assert_msg = "Length of list should be equal overall number of points!"
        assert len(self.test_array_by_points)==(self.x_max - self.x_min)*(self.y_max - self.y_min), assert_msg
